...
export function ForumPage() {
    ...
    useEffect(() => {
        axios
            .get(`${mainUrl}/api/v1/forum/getforum/${id}`)
            .then((response) => {
    ...
    const handleReviewSubmit = (e) => {
        ...
        axios
            ...
            .then((response) => {
                ...
                setForum((prevForum) => ({
                    ...prevForum,
                    reviews: [...(prevForum.reviews || []), response.data.review],
                }));
                setNewReview("");
                setRating(1);
        ...
        <div>
            ...
            <div className="container">
                <div className="p-5 h-20 bg-green-700 flex flex-col justify-center items-center">
                    <span className="px-1 text-lg bg-slate-100 rounded">
                        {forum.tag.name}
                    </span>
                ...
                <div className="mx-12 mt-5 p-2 flex border-2 border-black">
                    ...
                    <div>
                        <div className="ms-2">
                            {forum.user.name} {timeAgo(forum.createdAt)}
                        </div>
                        <div className="ps-4 mt-2 mb-2 text-xl font-bold">
                            {forum.title}
                        </div>
                    ...
                    {reviews.length > 0 ? (
                        reviews.map((review) => (
                            <div key={review._id} className="border-t border-gray-300 py-2">
                                <div>
                                    {review.user.name}: {review.review} (Rating: {review.rating})
                                </div>
